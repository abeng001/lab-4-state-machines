/*	Author: Alberto Bengo
 *  Partner(s) Name: Matthew Choi
 *	Lab Section: 23
 *	Assignment: Lab #4  Exercise #5
 *	Exercise Description: [optional - include for your own benefit]
 *
 *	I acknowledge all content contained herein, excluding template or example
 *	code, is my own original work.
 */
#include <avr/io.h>
#ifdef _SIMULATE_
#include "simAVRHeader.h"
#endif

typedef enum States { start, wait, PA2_press, PA2_release, Y_press, Y_release, Lock_press, X_press, X_release, unlocked } States;

int tick(int);

int main(void) {

	DDRA = 0x00; PORTA = 0xFF;
	DDRB = 0xFF; PORTB = 0x00;
	DDRC = 0xFF; PORTC = 0x00;

	States state = start;

	while (1) {
		state = tick(state);
	}
	return 1;
}

int tick(int state) {
	static unsigned char pb0;
	static unsigned char A0 = 0x01; //used in later exercises
	static unsigned char A1 = 0x02;
	static unsigned char A2 = 0x04;
	static unsigned char A7 = 0x80;
  static unsigned char code = 0x00;
	unsigned char input = PINA & 0xFF;


	switch (state) { // Transitions
	case start:
		state = wait;
		break;

	case wait:
		state = (input == A2) ? PA2_press : wait;
		break;

	case PA2_press:
		if (input == A2) {
			state = PA2_press;
		}
		else if (input == 0x00) {
			state = PA2_release;
		}
		else {
			state = wait;
		}
		break;

	case PA2_release:
		if (input == A0) {
			state = X_press;
		}
		else if (input == 0x00) {
			state = PA2_release;
		}
		else if (pb0 == 0x00) {
			state = wait;
		}
		else if (pb0 == 0x01) {
			state = unlocked;
		}
		break;

	case Y_press:
		if (input == A1) {
			state = Y_press;
		}
    else if (input == 0x00) {
      state = Y_release;
    }
		 else if ((input == 0x00) && (pb0 == 0x00)) {
		 	state = Y_release;
		 }
		 else if ((input == 0x00) && (pb0 == 0x01)) {
		 	state = wait;
		 }
		else if (pb0 == 0x00) {
			state = wait;
		}
		else if (pb0 == 0x01) {
			state = unlocked;
		}
		break;

	case Y_release:
		if (input == A7) {
			state = Lock_press;
		}
		else if(input == A2) {
			state = PA2_press;
		}
    else if (input == A0) {
      state = X_press;
    }
		else {
			state = Y_release;
		}
		break;

    case X_press:
      if (input == A0) {
        state = X_press;
      }
      else if ((input == 0x00) && (pb0 == 0x00)) {
  			state = X_release;
  		}
  		else if ((input == 0x00) && (pb0 == 0x01)) {
  			state = wait;
  		}
      else if (pb0 == 0x00) {
        state = wait;
      }
      else if (pb0 == 0x01) {
        state = unlocked;
      }
      break;

    case X_release:
      if (input == A1) {
  			state = Y_press;
  		}
      else if (input == A7) {
        state = Lock_press;
      }
      else if (input == A2) {
        state = PA2_press;
      }
  		else if (input == 0x00) {
  			state = X_release;
  		}
  		else if (pb0 == 0x00) {
  			state = wait;
  		}
  		else if (pb0 == 0x01) {
  			state = unlocked;
  		}
      if (code == 0x09) {
        state = unlocked;
      }
      break;

	case Lock_press:
		if (input == 0x00) {
			state = wait;
		}
		else {
			state = Lock_press;
		}
		break;

  case unlocked:
    if (input == A7) {
      state = Lock_press;
      code = 0;
    }
    else if(input == A2) {
      state = PA2_press;
      code = 0;
    }
    else if (input == A1) {
      state = Y_press;
    }
    else if (input == A0) {
      state = X_press;
    }
    else if (input == 0x00) {
      state = wait;
    }
    else {
      state = unlocked;
    }
    break;

	default:
		state = start;
		break;
	}

	switch (state) { // Actions
	case start: break;

	case wait:
    if (code == 0x09) {
      state = unlocked;
    }
    else {
      pb0 = 0x00;
      code = 0x00;
    }
		break;

	case PA2_press: break;

	case PA2_release:
    if (code == 0) {
      code = code + 1;
    }
    break;

	case Y_press: break;

	case Y_release:
    if (code == 4) {
      code = code + 2;
    }
    break;

  case X_press: break;

  case X_release:
    if (code == 1 || code == 6) {
      code = code + 3 ;
    }
    break;

	case Lock_press: break;

  case unlocked:
    pb0 = 0x01;
    break;
	}

	PORTB = pb0;
	PORTC = state;
	return state;
}
